...
2. **List attribute changes.** Stage moved on a pipeline list? That's a list-entry change — use [List entry updated](/reference/triggers/list-entry-updated/). See [records vs list entries](/explanation/records-vs-list-entries/).

## Gotchas

- **Unscoped = run-volume firehose.** Every enrichment pass, every sync, every bulk edit fires it for every touched record. Unscoped Record updated workflows are the leading source of mystery credit burn and [trigger loops](/explanation/infinite-loops-and-safety/).
...
[Attribute value changed](/reference/triggers/attribute-value-changed/) · [List entry updated](/reference/triggers/list-entry-updated/) · [Trigger guide](/explanation/triggers/)
