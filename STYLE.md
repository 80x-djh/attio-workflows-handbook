...
| Helpful tip | `<Tip>**Lead.** …</Tip>` (callouts take children only, so fold any title into a bold lead) |
...
2. **Availability & cost table** (no heading, immediately after the summary), with fixed rows in fixed order: **Category**, **Credit cost**, **Engine** (New Workflows engine, June 2026+), **Plan availability**, **Limits**.
...
4. One idea per sentence, and keep sentences short. Split anything past ~22 words, or any sentence that stacks clauses with commas and "and". Plain words beat clever ones. Lead with the simple statement, then add the nuance in its own sentence. Do not assume prior knowledge: explain or link a domain term the first time the page uses it.
...
7. Never "above", "below", "here", or "click here". Link text equals the target page title or reads as a natural noun phrase. Never use a bare URL as link text.
8. Serial comma, American English, straight quotes; contractions are fine. **No em dashes** (the long dash character). Use a period (split the sentence, preferred), a comma, a colon, or parentheses instead.
9. Expand acronyms on first use per page (MCP, ICP, PQL), except CRM, API, and URL. Link engine jargon to the glossary on first mention.
